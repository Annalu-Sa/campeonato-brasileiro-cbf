...
int nomeExiste(Equipe lista[], int total, char nomeProcurado[]) {
    for (int i = 0; i < total; i++) {
        if (strcmp(lista[i].nome, nomeProcurado) == 0) {
            //Se houver um nome igual ele devolve o numero do indice dessa equipe no array de structs de equipe
            //assim podemo modificar a equipe caso o usuario queira
            return i;
        }
    }
    return -1;
...
bool siglaExiste(Equipe lista[], int total, char siglaProcurada[], int indiceModificando) {
    for (int i = 0; i < total; i++) {
        //Se estiver modificando uma equipe criada anteriormente, quando for veridicar se a sigla já existe,
        //o codigo nao comete o erro de olhar a sigla anterior criada para a função que está sendo modificada
        //assim se o usuario não quiser mudar a sigla ele pode só escreve-la de novo e não dará erro
        if(i == indiceModificando) {
            continue;
        }
        else if (strcmp(lista[i].sigla, siglaProcurada) == 0) {
            return true;
        }
    }
    return false;
...
Sistema cadastroEquipe(Sistema sistema) {
    ...
    //Pega o indice da equipe (caso exista). Se retornar -1 não existe equipe com esse nome;
    //se retornar >0 ele existe e o indice é o numero retornado
    int indiceEquipe = nomeExiste(sistema.equipes, sistema.numEquipesCadastradas, nova.nome);
    bool estou_atualizando = false;

    //Avisa que o nome ja existe e pergunta se o usuario quer modificalo ou cancelar a operaçao
    if (indiceEquipe != -1) {
        printf("-----------------------------\n");
        printf("Aviso: A equipe '%s' ja esta cadastrada.\n", nova.nome);
        printf("Deseja atualizar os dados dessa equipe? (S/N): ");
        char confirmacao;
        scanf("%c", &confirmacao);
        limpar_buffer();
    ...
    while (true) {
        ...
        if (nova.sigla[tam_sigla - 1] == '\n') {
            //tira o \n e adiciona o '\0' ao fim para fechar
        ...
        para_maiusculo(nova.sigla);//deixa a sigla sempre em maiusculo

        //Verifica se o usuario está atualizando ou nao uma equipe
        //se a variavel estou_atualizando for verdadeira usa o valor da variavel indiceEquipe
        //do contrário(falso) usa -1
        int indiceModificando = estou_atualizando ? indiceEquipe : -1;

        if (tam_sigla != 3) {
            printf("Erro: A sigla precisa ter EXATAMENTE 3 letras.\n");
            printf("Digite a sigla da equipe:\n");
        }
        //agora usa a função siglaExiste levando em conta o indice da própria equipe que está sendo modificada
        //e o ignora para não acusar que essa sigla já existe caso o usuario queira manter
        else if (siglaExiste(sistema.equipes, sistema.numEquipesCadastradas, nova.sigla, indiceModificando)) {
    ...
    if(estou_atualizando){
        // Substitui apenas os dados que mudamos(Nome, Sigla, Estado), mas mantendo as estatísticas do placar
        strcpy(sistema.equipes[indiceEquipe].sigla, nova.sigla);
        strcpy(sistema.equipes[indiceEquipe].estado, nova.estado);
        printf("\nDados da equipe '%s' atualizados com sucesso para:\n| Sigla [%s] |\n| UF [%s] |\n", nova.nome, nova.sigla, nova.estado);
    } else {
        // Inicializa estatísticas de uma NOVA equipe
        nova.vitorias = 0;
        nova.empates = 0;
        nova.derrotas = 0;
        nova.golsMarcados = 0;
